...
export default function App() {
  ...
  const isButtonDisabled = !input.trim() || !ws || ws.readyState !== 1
  
  // Update debug info when state changes (not on every render)
  useEffect(() => {
    updateDebug(`Button disabled: ${isButtonDisabled} (input: "${input.trim()}", ws: ${!!ws}, readyState: ${ws?.readyState})`)
  }, [isButtonDisabled, input, ws?.readyState])

  return (
    <div className="h-screen flex flex-col bg-gray-100 p-4">
      {/* Debug info panel */}
      <div className="bg-yellow-100 border border-yellow-400 rounded p-2 mb-2 text-xs">
      ...
      <div className="mt-2 flex">
        ...
        <button 
          ...
          className={`px-4 rounded-r-xl transition-colors ${
            isButtonDisabled 
      ...
      <div className="mt-1 text-xs text-gray-500">
        ...
        Input: "{input}" | 
        Disabled: {isButtonDisabled ? 'YES' : 'NO'}
      </div>
    </div>
  )
}
