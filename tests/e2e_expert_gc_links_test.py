...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_expert_gc_links(monkeypatch):
    ...
    async with websockets.connect(uri) as ws:
        await ws.send(json.dumps({"type": "message", "content": "как настроить DLP Infowatch?"}))
        got = ""
        for _ in range(30):
            ...
            if msg.get("type") == "chat" and msg.get("content"):
                got += msg["content"] + "\n"
        assert "FINAL_ANSWER:" in got
        assert "### Ссылки" in got
        assert "https://" in got
